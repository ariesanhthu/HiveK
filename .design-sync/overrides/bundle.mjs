// forked from design-sync lib/bundle.mjs — HiveK: stub Next.js server-only
// modules (server-only, client-only, node built-ins) so client feature
// components that transitively reach server/data code still bundle for the
// browser. Output contract (IIFE header/export shape) is unchanged; this only
// resolves server leaves to empty no-op modules a client render never calls.
...
// HiveK fork: Next.js server-only guard modules and node built-ins that leak
...
// stub these CSS side-effect imports to empty modules.
const CSS_STUB = /(^tailwindcss$|\.css$)/;
export const serverStub = {
  name: 'server-stub',
  setup(b) {
    b.onResolve({ filter: SERVER_STUB }, (a) => ({
      path: a.path,
      // next/font/* exposes NAMED font factories (Inter, Roboto…) called at
      // module top level. esbuild's __toESM interop copies only a CJS module's
      // own enumerable keys, and a Proxy exposes none — so a generic Proxy stub
      // yields `undefined` named imports that crash on call. Route font modules
      // to a loader that declares those names as real ESM exports.
      namespace: /^next\/font\//.test(a.path) ? 'font-stub' : 'server-stub',
    }));
    b.onResolve({ filter: CSS_STUB }, (a) => ({ path: a.path, namespace: 'server-stub' }));
    // A font factory returns { className, variable, style }; all stubbed empty —
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [serverStub, reactShim];
