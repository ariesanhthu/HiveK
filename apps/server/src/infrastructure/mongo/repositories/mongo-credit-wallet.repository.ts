...
export class MongoCreditWalletRepository implements ICreditWalletRepository {
  ...
  async findById(id: string): Promise<Nullable<CreditWalletRoot>> {
    const doc = await this.walletModel.findById(id).session(this.session).exec();
    return doc ? this.mapToDomain(doc) : null;
  }

  async findByEnterpriseId(enterpriseId: string): Promise<Nullable<CreditWalletRoot>> {
    const doc = await this.walletModel.findOne({ enterprise_id: enterpriseId }).session(this.session).exec();
    return doc ? this.mapToDomain(doc) : null;
  }
  ...
  async save(wallet: CreditWalletRoot): Promise<void> {
    ...
    if (!wallet.id) {
      ...
      await this.walletModel.findByIdAndUpdate(wallet.id, data, { upsert: true }).session(this.session).exec();
    }
  ...
  private mapToDomain(doc: CreditWalletDocument): CreditWalletRoot {
    return CreditWalletRoot.instantiate(
      doc._id.toString(),
      {
  ...
  private mapToPersistence(data: CreditWalletRoot): Omit<CreditWalletModel, 'created_at' | 'updated_at'> {
    return {
