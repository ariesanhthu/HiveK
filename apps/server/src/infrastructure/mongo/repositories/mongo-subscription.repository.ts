...
export class MongoSubscriptionRepository implements ISubscriptionRepository {
  ...
  async save(subscription: SubscriptionRoot): Promise<void> {
    ...
    if (!subscription.id) {
      ...
      await this.subscriptionModel
        .findByIdAndUpdate(subscription.id, data, { upsert: true })
        .session(this.session)
        .exec();
    }
  ...
  private mapToDomain(doc: SubscriptionDocument): SubscriptionRoot {
    return SubscriptionRoot.instantiate(doc._id.toString(), {
      userId: doc.user_id,
      status: doc.status,
  ...
  private mapToPersistence(
    data: SubscriptionRoot,
  ): Omit<SubscriptionModel, 'created_at' | 'updated_at'> {
    return {
      user_id: data.userId,
      status: data.status,
