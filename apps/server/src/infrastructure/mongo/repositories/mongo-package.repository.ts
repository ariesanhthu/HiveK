...
export class MongoPackageRepository implements IPackageRepository {
  ...
  async save(pkg: PackageRoot): Promise<void> {
    ...
    if (!pkg.id) {
      ...
      await this.packageModel
        .findByIdAndUpdate(pkg.id, data, { upsert: true })
        .session(this.session)
        .exec();
    }
  ...
  async deleteByCode(code: string): Promise<void> {
    ...
    await Promise.all(
      docs.map((doc) =>
        this.invalidateCache(doc._id.toString(), doc.code, doc.enterprise_id),
      ),
    );
  }

  private mapToDomain(doc: PackageDocument): PackageRoot {
    return PackageRoot.instantiate(doc._id.toString(), {
      code: doc.code,
      name: doc.name,
      ...
      variants: (doc.variants || []).map(
        (v: PackageDocument['variants'][0]) => {
          return PackageVariantEntity.instantiate(v._id.toString(), {
            title: v.title,
  ...
  private mapToPersistence(
    data: PackageRoot,
  ): Omit<PackageModel, 'created_at' | 'updated_at'> {
