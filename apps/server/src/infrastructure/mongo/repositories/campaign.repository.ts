  ...
  async findByOutputId(outputId: string): Promise<Nullable<CampaignRoot>> {
    ...
    const doc = await this.campaignModel
      .findOne({
        $or: [
          {
            'schedule.timeline.posts.campaign_kol_outputs._id':
              new Types.ObjectId(outputId),
          },
          {
            'schedule.timeline.posts.campaign_enterprise_outputs._id':
              new Types.ObjectId(outputId),
          },
        ],
      })
  ...
  private mapToDomain(doc: CampaignDocument): CampaignRoot {
    ...
    return CampaignRoot.instantiate(doc._id.toString(), {
      ...
      participants: (doc.participants || []).map(
        (p: CampaignDocument['participants'][0]) =>
      ...
      schedule: doc.schedule
        ? {
            timeline: (doc.schedule.timeline || []).map(
              (day: CampaignDocument['schedule']['timeline'][0]) => ({
                date: day.date,
                label: day.label,
