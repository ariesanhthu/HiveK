...
import { CampaignParticipantModel, CampaignParticipantDocument } from '../schemas/campaign-participant.schema';
...
export class MongoCampaignParticipantRepository implements ICampaignParticipantRepository {
  ...
  async findByCampaignAndKol(
    ...
    const doc = await this.participantModel
      .findOne({
        ...
        kol_profile_id: new Types.ObjectId(kolProfileId) as any,
      })
      .exec();
    return doc ? this.mapToDomain(doc) : null;
  }
  ...
  async findByOutputId(outputId: string): Promise<Nullable<CampaignParticipantRoot>> {
    const doc = await this.participantModel
      .findOne({
        'outputs._id': new Types.ObjectId(outputId) as any,
      })
      .exec();
    return doc ? this.mapToDomain(doc) : null;
  }
  ...
  private mapToPersistence(root: CampaignParticipantRoot): any {
    return {
