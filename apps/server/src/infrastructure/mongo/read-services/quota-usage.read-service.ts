...
export class MongoQuotaUsageReadService implements IQuotaUsageReadService {
  ...
  async findAll(
    filters: QuotaUsageFilterDto = {},
  ): Promise<PaginatedResponseDto<QuotaUsageResponseDto>> {
  ...
  private mapToDto(
    doc: FlattenMaps<QuotaUsageDocument>,
    ...
    return {
      ...
      cycleAnchorDate:
        doc.cycle_anchor_date instanceof Date
          ? doc.cycle_anchor_date.toISOString()
          : new Date(doc.cycle_anchor_date).toISOString(),
      usages: (doc.usages || []).map((u) => ({
        key: u.key,
        allocated: u.allocated,
        used: u.used,
        cycleStartAt:
          u.cycle_start_at instanceof Date
            ? u.cycle_start_at.toISOString()
            : new Date(u.cycle_start_at).toISOString(),
        cycleEndsAt:
          u.cycle_ends_at instanceof Date
            ? u.cycle_ends_at.toISOString()
            : new Date(u.cycle_ends_at).toISOString(),
      })),
      updatedAt:
