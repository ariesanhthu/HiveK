...
const errorCodes: number[] = Object.keys(momoResponseStatusCodeMapper)
  .map(Number)
  .filter(
    (code) => !successCodes.includes(code) && !processingCodes.includes(code),
  );

export function getMomoResultGroup(
  code: number,
): 'success' | 'processing' | 'error' | 'unknown' {
...
export function getMomoResultMessage(code: number): string {
  if (momoResponseStatusCodeMapper[code])
    return momoResponseStatusCodeMapper[code];
