...
export interface IOtpRepository {
  ...
  findValidOtp(email: string, code: string, type: EOtpType): Promise<OtpRoot | null>;
  deleteByEmailAndType(email: string, type: EOtpType): Promise<void>;
  findRecentOtp(email: string, type: EOtpType, withinSeconds: number): Promise<OtpRoot | null>;
}
