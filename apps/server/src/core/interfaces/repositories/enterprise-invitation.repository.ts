...
export interface IEnterpriseInvitationRepository extends IBaseRepository<EnterpriseInvitationRoot> {
  findByEmailAndEnterpriseId(
    email: string,
    enterpriseId: string,
  ): Promise<EnterpriseInvitationRoot | null>;
  findPendingByEmailAndEnterpriseId(
    email: string,
    enterpriseId: string,
  ): Promise<EnterpriseInvitationRoot | null>;
...
export const ENTERPRISE_INVITATION_REPOSITORY = Symbol(
  'IEnterpriseInvitationRepository',
);
