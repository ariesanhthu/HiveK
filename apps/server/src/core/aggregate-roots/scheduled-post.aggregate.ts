...
export class ScheduledPostRoot extends BaseAggregateRoot<ScheduledPostProps> {
  ...
  public static create(
    props: ScheduledPostCreateProps,
    id?: string,
  ): ScheduledPostRoot {
    const now = new Date();
  ...
  get mediaFileIds(): FileId[] {
    return [...this.props.mediaFileIds];
  }
  get scheduledAt(): Date {
    return this.props.scheduledAt;
  }
  get status(): EPostStatus {
    return this.props.status;
  }
  get publishedAt(): Nullable<Date> {
    return this.props.publishedAt;
  }
  get platformPostId(): Nullable<string> {
    return this.props.platformPostId;
  }
  get failReason(): Nullable<string> {
    return this.props.failReason;
  }
  ...
  public schedule(scheduledAt: Date): void {
    if (this.props.status !== EPostStatus.DRAFT) {
      throw new InvalidOperationException(
        'Post must be in draft status to schedule.',
      );
    }
  ...
  public markPublishing(): void {
    if (this.props.status !== EPostStatus.SCHEDULED) {
      throw new InvalidOperationException(
        'Post must be in scheduled status to start publishing.',
      );
    }
  ...
  public markPublished(
    ...
    if (
      ...
      throw new InvalidOperationException(
        'Post must be in publishing or scheduled status to mark as published.',
      );
    }
  ...
  public markFailed(reason: string): void {
    if (
      ...
      throw new InvalidOperationException(
        'Post must be in publishing or scheduled status to mark as failed.',
      );
    }
  ...
  public cancel(): void {
    if (
      this.props.status !== EPostStatus.DRAFT &&
      this.props.status !== EPostStatus.SCHEDULED
    ) {
      throw new InvalidOperationException(
        'Post can only be cancelled from draft or scheduled status.',
      );
    }
