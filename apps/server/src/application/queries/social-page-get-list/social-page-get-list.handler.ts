...
export class SocialPageGetListHandler implements IQueryHandler<SocialPageGetListQuery, SocialPageDto[]> {
  constructor(
  ...
  async execute(query: SocialPageGetListQuery): Promise<SocialPageDto[]> {
    const pages = await this.socialPageRepository.findByEnterpriseId(query.enterpriseId);
