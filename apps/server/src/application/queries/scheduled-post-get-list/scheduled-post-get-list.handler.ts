...
export class ScheduledPostGetListHandler implements IQueryHandler<ScheduledPostGetListQuery, ScheduledPostDto[]> {
  constructor(
  ...
  async execute(query: ScheduledPostGetListQuery): Promise<ScheduledPostDto[]> {
    const posts = await this.scheduledPostRepository.findByEnterpriseId(query.enterpriseId);
