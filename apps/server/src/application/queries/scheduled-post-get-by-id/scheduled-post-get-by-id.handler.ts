...
export class ScheduledPostGetByIdHandler implements IQueryHandler<ScheduledPostGetByIdQuery, ScheduledPostDto> {
  constructor(
  ...
  async execute(query: ScheduledPostGetByIdQuery): Promise<ScheduledPostDto> {
    ...
    if (post.enterpriseId !== query.enterpriseId) {
      throw new InvalidOperationException('Unauthorized enterprise access to post.');
    }
