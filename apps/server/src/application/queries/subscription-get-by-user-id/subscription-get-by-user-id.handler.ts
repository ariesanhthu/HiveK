...
export class SubscriptionGetByUserIdHandler implements IQueryHandler<SubscriptionGetByUserIdQuery> {
  ...
  async execute(query: SubscriptionGetByUserIdQuery): Promise<SubscriptionResponseDto> {
    const dto = await this.readService.findByUserId(query.userId);
    if (!dto) {
      throw new NotFoundException(`Subscription for user ID ${query.userId} not found`);
    }
    return dto;
  }
}
