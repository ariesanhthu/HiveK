...
export class PaymentProviderGetByIdHandler implements IQueryHandler<PaymentProviderGetByIdQuery, PaymentProviderResponseDto | null> {
  constructor(
    @Inject(PAYMENT_PROVIDER_READ_SERVICE)
    private readonly readService: IPaymentProviderReadService,
  ) {}

  async execute(query: PaymentProviderGetByIdQuery): Promise<PaymentProviderResponseDto | null> {
    const { input } = query;
