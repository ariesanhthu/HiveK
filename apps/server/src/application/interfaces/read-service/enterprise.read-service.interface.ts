...
export interface IEnterpriseReadService extends IBaseReadService<EnterpriseDetailDto, EnterpriseFilterDto> {
  findByUserId(userId: string): Promise<Nullable<EnterpriseDetailDto>>;
  findByUserIdOrMember(userId: string, filters?: EnterpriseFilterDto): Promise<PaginatedResponseDto<EnterpriseDetailDto>>;
}
