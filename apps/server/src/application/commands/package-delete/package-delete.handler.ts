...
export class PackageDeleteHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: PackageDeleteCommand): Promise<void> {
    ...
    return this.uow.execute(async () => {
      ...
      // 2. Validate Status
      if (
        pkg.status !== EVersionStatus.DRAFT &&
        pkg.status !== EVersionStatus.ARCHIVED
      ) {
        throw new Error(
      ...
      if (pkg.status === EVersionStatus.ARCHIVED) {
        const isUsed = await this.subscriptionRepository.existsByPackageId(
          pkg.id,
        );
        if (isUsed) {
