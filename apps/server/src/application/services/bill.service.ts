...
export class BillService {
  ...
  async determinePurchaseTypes(
    ...
    for (const item of requestedItems) {
      ...
      const lineType = isPlan
        ? EBillLineType.PLAN_PURCHASE
        : EBillLineType.ADDON_PURCHASE;

      const existingPlan =
        subscription?.planItem?.packageVariantId === variant.id;
      const existingAddon =
        subscription?.addonItems.some(
          (i) => i.packageVariantId === variant.id,
        ) || false;

      const existingItem = isPlan ? existingPlan : existingAddon;
      const purchaseType = existingItem
        ? EPurchaseType.RENEWAL
        : EPurchaseType.NEW;
