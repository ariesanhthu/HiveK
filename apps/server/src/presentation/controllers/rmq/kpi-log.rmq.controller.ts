...
export class KpiLogRmqController {
  ...
  async handleTrackingSuccess(data: any) {
    this.logger.log(`📥 Received tracking.success via RMQ: ${JSON.stringify(data)}`);
    try {
      await this.commandBus.execute(new KpiLogCreateCommand(data));
    } catch (error) {
      this.logger.error(`Error processing tracking.success: ${error.message}`, error.stack);
  ...
  async handleTrackingTerminated(data: any) {
    this.logger.log(`📥 Received tracking.terminated via RMQ: ${JSON.stringify(data)}`);
    try {
      await this.commandBus.execute(new KpiLogTerminateCommand(data));
    } catch (error) {
      this.logger.error(`Error processing tracking.terminated: ${error.message}`, error.stack);
      throw error;
    }
  }
}
