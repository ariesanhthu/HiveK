...
import { KPI_LOG_REPOSITORY, CAMPAIGN_PARTICIPANT_REPOSITORY } from '@core/interfaces/repositories';
import { createMockWebSocketService } from '../../../__mocks__/mock-services';
import { createMockKpiLogRepository, createMockCampaignParticipantRepository } from '../../../__mocks__/mock-repositories';
...
describe('KpiTrackingEventsHandlers', () => {
  ...
  describe('KpiMetricsUpdatedWsHandler', () => {
    ...
    it('should emit websocket event when participant and log exist', async () => {
      participantRepository.findById.mockResolvedValue({ id: 'participant-1', kolProfileId: 'kol-1', campaignId: 'campaign-1' });
    ...
    it('should not emit if kpi log not found', async () => {
      participantRepository.findById.mockResolvedValue({ id: 'participant-1', kolProfileId: 'kol-1' });
