...
describe('CreatePlatformHandler', () => {
  ...
  it('should create platform successfully', async () => {
    const input = {
      ...
      apiStatus: 'active',
      iconUrl: 'https://facebook.com/icon.png',
    };
  ...
  it('should throw error if platform name already exists', async () => {
    const input = {
      name: 'Facebook',
      baseUrl: 'https://facebook.com',
      apiStatus: 'active',
    ...
    await expect(handler.execute(command)).rejects.toThrow('Platform with name Facebook already exists');
