...
1. **Pre-filtering**: Add a filtering step before the loop or bulk operation to identify which users are _actually_ new additions to the enterprise.
...
1. **Strict Validation**: Add a validation step _before_ creating the enterprise entity to ensure the user exists and is a valid `EnterpriseUserRoot`. If the validation fails, throw an appropriate exception and halt creation.
