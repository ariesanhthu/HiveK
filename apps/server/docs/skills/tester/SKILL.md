...
- _CRITICAL:_ You do not have access to the Handlers (the implementation). Do not assume the code works correctly.
...
3. **Identify Hidden Handler Workflow:** Deduce what the hidden handler _should_ do (e.g., fetch via repository interface, apply domain method, save, emit event).
4. **Generate Matrix:**
...
Provide _only_ clean, production-grade TypeScript code using `@nestjs/testing` and `jest`.
