...
import { MoneyVO, type PaymentAttemptStatusVO, FailureTypeVO } from '../value-objects';
import { type PaymentTransactionEntity } from './payment-transaction.entity';
import { type Nullable, type Optional } from '@/core/types';
import { type FieldChanges } from './payment-event.entity';
...
export class PaymentAttemptEntity extends Entity<PaymentAttemptProps> {
	...
	public getCapturedAmount(): MoneyVO {
		// Get currency from first transaction, default to VND if no transactions
	...
	public addTransaction(transaction: PaymentTransactionEntity): void {
		// Enforce currency consistency - all transactions must use same currency
		...
		// Update provider transaction ID if this is a success transaction
		if (
	...
	public transitionStatus(): void {
		...
		switch (lastTransaction.transactionType) {
			case EPaymentTransactionType.CREATE:
				// CREATE successful - payment URL generated, waiting for user
			...
			case EPaymentTransactionType.REFUND:
				// Partial vs full refund determined by amount comparison
	...
	/**
	 * Check if this attempt can be retried based on failure type
	 */
	public canRetry(): boolean {
		// No failure yet = can retry
