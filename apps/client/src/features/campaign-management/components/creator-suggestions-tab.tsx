...
export function CreatorSuggestionsTab({
  ...
  const [contactCreator, setContactCreator] = useState<CampaignCreatorSuggestion | null>(null);
              ...
              >
                <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
                  ...
                  <div className='grid shrink-0 grid-cols-2 gap-2 text-xs text-foreground-muted lg:w-64'>
                    <p>
                      Follower:{' '}
                      <span className='font-bold text-foreground'>{creator.followerRange}</span>
                    </p>
                    <p>
                      Engagement:{' '}
                      <span className='font-bold text-foreground'>{creator.engagementRate}%</span>
                    </p>
                    <p className='col-span-2'>
                      Cost:{' '}
                      <span className='font-bold text-foreground'>{creator.estimatedCost}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className='mt-4 rounded-lg bg-background-light p-3'>
                  ...
                  <ul className='mt-2 space-y-1 text-xs leading-5 text-foreground-muted'>
                    {creator.reason.map((reason) => <li key={reason}>- {reason}</li>)}
                  </ul>
                </div>
