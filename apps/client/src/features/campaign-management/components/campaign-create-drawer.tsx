          ...
          <header className="flex shrink-0 items-start justify-between gap-4 border-b border-primary-soft px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-foreground-muted">
                Nhập thông tin để AI hiểu mục tiêu, khách hàng, tone giọng và tài nguyên của chiến dịch.
              </p>
            </div>
            <button
              type="button"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Mục tiêu chiến dịch *">
                  <select
                    ...
                    onChange={(event) =>
                      updateForm("objective", event.target.value as CampaignFormInput["objective"])
                    }
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <Field label="Mức độ trang trọng: 1 - 5">
                    <Input
                      ...
                      onChange={(event) =>
                        updateForm("formality", Number(event.target.value) as CampaignFormInput["formality"])
                      }
                    />
                  </Field>
                  <Field label="Emoji">
                    <select
                      ...
                      onChange={(event) =>
                        updateForm("emojiLevel", event.target.value as CampaignFormInput["emojiLevel"])
                      }
                ...
                <div className="rounded-xl border border-primary-soft bg-background-light p-4">
                  ...
                  <div className="mt-3 grid gap-3 text-sm text-foreground-muted md:grid-cols-2">
                    <label className="flex items-center gap-2">
                      <input type="checkbox" defaultChecked /> Dùng media này cho toàn bộ chiến dịch
                    </label>
                    <label className="flex items-center gap-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Số bài muốn tạo *">
                  <Input
                    ...
                    onChange={(event) =>
                      updateForm("numberOfPosts", Number(event.target.value))
                    }
                  />
                </Field>
                ...
                <Field label="Mức sáng tạo: 1 - 5">
                  <Input
                    ...
                    onChange={(event) =>
                      updateForm("creativity", Number(event.target.value) as CampaignFormInput["creativity"])
                    }
                  />
                </Field>
                <Field label="Chế độ duyệt">
                  <select
                    ...
                    onChange={(event) =>
                      updateForm("approvalMode", event.target.value as CampaignFormInput["approvalMode"])
                    }
                    className="h-11 w-full rounded-xl border border-primary-soft bg-muted/60 px-3 text-sm"
                  >
                ...
                <div className="md:col-span-2 rounded-xl border border-primary-soft bg-background-light p-4">
                  ...
                  <div className="mt-3 grid gap-3 text-sm text-foreground-muted md:grid-cols-3">
                    {["Caption", "Hashtag", "CTA", "Media prompt", "Lịch đăng", "Tin nhắn KOL/KOC"].map(
                      (item) => (
