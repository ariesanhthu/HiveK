...
export function KolComparisonTable({
  ...
  const highestFitScore = activeCandidates.length > 0
    ? Math.max(...activeCandidates.map((c) => c.fitScore))
    : 0;
  const lowestFee = activeCandidates.length > 0
            ...
            <table className='w-full min-w-[760px] table-fixed border-collapse lg:min-w-0'>
                      ...
                      >
                        <div className='flex items-start justify-between gap-2'>
                          ...
                          <button
                            ...
                            aria-label={excluded
                              ? 'Restore this creator in the comparison'
                              : 'Hide this column (dimmed)'}
                            aria-pressed={excluded}
                          >
                            <span
                              className='material-symbols-outlined text-[1.125rem] leading-none'
                              aria-hidden
                            >
                              {excluded ? 'undo' : 'close'}
                            </span>
              ...
              <tbody className='[&_tr>td]:transition-colors [&_tr:hover>td:not([data-excluded])]:bg-primary-soft'>
                <tr className='odd:bg-card even:bg-muted/60'>
                  <td className={`${METRIC_LABEL_CELL_CLASS} font-semibold text-foreground`}>
                    Điểm phù hợp
                  </td>
                  {candidates.map((candidate) => {
                    const excluded = isColumnExcluded(candidate.id);
                    const isTop = !excluded
                ...
                <tr className='odd:bg-card even:bg-muted/60'>
                  ...
                  {candidates.map((candidate) => (
                    ...
                    >
                      <p className={METRIC_NUMBER_TEXT_CLASS}>
                ...
                <tr className='odd:bg-card even:bg-muted/60'>
                  ...
                  {candidates.map((candidate) => (
                    ...
                    >
                      <p className={METRIC_VALUE_TEXT_CLASS}>
                ...
                <tr className='odd:bg-card even:bg-muted/60'>
                  ...
                  {candidates.map((candidate) => {
                    ...
                    return (
                      ...
                      >
                        <p className={METRIC_NUMBER_TEXT_CLASS}>
                          {formatMoney(candidate.estimatedCostPerPostUsd)}
                        </p>
                      </td>
                    );
                  })}
