...
export function CampaignCard({ campaign }: CampaignCardProps) {
  ...
  if (campaign.statusDetail.includes('Hạn chót')) {
    detailTextClass = 'text-red-500 font-bold bg-red-50 px-2 py-0.5 rounded-full text-[10px]';
  } else if (
    campaign.statusDetail.includes('phê duyệt') || campaign.statusDetail.includes('THANH TOÁN')
  ) {
    detailTextClass =
      'text-emerald-500 font-bold bg-emerald-50 px-2 py-0.5 rounded-full text-[10px]';
  ...
  const handleCopyLink = async () => {
    try {
      ...
      const res = await fetch(
        `/api/shortlink?productSlug=${encodeURIComponent(campaign.productSlug)}&kolId=${
          encodeURIComponent(CURRENT_KOL_ID)
        }`,
      );
      ...
      const link =
        `${window.location.origin}/certificate-product/${campaign.productSlug}-${CURRENT_KOL_ID}`;
  ...
  return (
    ...
    >
          ...
          <div className='h-2 w-full overflow-hidden rounded-full bg-slate-100'>
            <div
              className={cn(
                ...
                campaign.shortStatus === 'ĐANG ĐĂNG BÀI'
                  ? 'bg-[#f39c12]'
                  : campaign.shortStatus === 'CHỜ DUYỆT'
                  ? 'bg-blue-500'
                  : campaign.shortStatus === 'ĐANG LÊN KẾ HOẠCH'
                  ? 'bg-purple-500'
                  : 'bg-emerald-400',
              )}
      ...
      <div className='flex flex-col gap-2 md:w-[160px] flex-shrink-0'>
        <button
          className={cn(
            'w-full rounded-xl py-2.5 text-sm font-bold transition-transform active:scale-95',
            primaryBtnClass,
          )}
        >
          {primaryBtnText}
