    ...
    <div className="h-full overflow-y-auto bg-card">
      <div className="border-b border-primary-soft p-5 text-center">
        <ChannelAvatar
          ...
          className="mx-auto"
        />
        <h2 className="mt-3 text-base font-extrabold text-foreground">{conversation.customer.name}</h2>
      ...
      <ContextSection icon={<UserRound className="size-4" aria-hidden />} title="Phân loại">
        <dl className="space-y-2 text-xs">
          ...
          <div className="flex justify-between gap-3">
            ...
            <dd className="text-right font-bold text-foreground">{conversation.customer.leadStage}</dd>
          </div>
        </dl>
        <div className="mt-3 flex flex-wrap gap-1.5">
          {conversation.tags.map((item) => (
            <span key={item} className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-1 text-[10px] font-bold text-foreground-muted">
      ...
      <ContextSection icon={<BriefcaseBusiness className="size-4" aria-hidden />} title="Ngữ cảnh kinh doanh">
      ...
      <ContextSection icon={<Bot className="size-4" aria-hidden />} title="AI Agent">
        <dl className="space-y-2 text-xs">
          <div className="flex justify-between gap-3">
            ...
            <dd className="text-right font-bold text-foreground">{HANDLING_MODE_LABELS[conversation.handlingMode]}</dd>
          </div>
          <div className="flex justify-between gap-3">
            <dt className="text-foreground-muted">Trạng thái</dt>
            <dd className="text-right font-bold text-foreground">{AI_STATE_LABELS[conversation.aiState]}</dd>
          </div>
        </dl>
      ...
      <ContextSection icon={<UserRound className="size-4" aria-hidden />} title="Quản lý">
        <label className="block text-[11px] font-bold text-foreground-muted" htmlFor="inbox-assignee">
          Người phụ trách
        </label>
        <select
          id="inbox-assignee"
        ...
        <label className="mt-3 block text-[11px] font-bold text-foreground-muted" htmlFor="inbox-status">
          Trạng thái
        </label>
        <select
          id="inbox-status"
