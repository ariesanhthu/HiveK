...
export function ConversationPanel({
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-card" aria-label={`Hội thoại với ${conversation.customer.name}`}>
      <header className="shrink-0 border-b border-primary-soft bg-card px-3 py-3 sm:px-4">
        <div className="flex items-center gap-3">
          <Button size="icon" variant="ghost" className="size-9 md:hidden" onClick={onBack} aria-label="Quay lại danh sách hội thoại">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-sm font-extrabold text-foreground">{conversation.customer.name}</h2>
          ...
          <Button
            size="icon"
            variant="ghost"
            className={cn("size-9", conversation.priority === "urgent" ? "text-amber-600" : "text-foreground-muted")}
            onClick={onTogglePriority}
            aria-label={conversation.priority === "urgent" ? "Bỏ đánh dấu ưu tiên" : "Đánh dấu ưu tiên"}
          >
            <Star className="size-4" fill={conversation.priority === "urgent" ? "currentColor" : "none"} aria-hidden />
          </Button>
          <Button size="icon" variant="ghost" className="size-9 2xl:hidden" onClick={onOpenContext} aria-label="Mở thông tin khách hàng">
            <Info className="size-4" aria-hidden />
          </Button>
          <Button size="icon" variant="ghost" className="hidden size-9 sm:inline-flex" aria-label="Thêm hành động">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-foreground-muted">Phụ trách: {conversation.assignee}</span>
