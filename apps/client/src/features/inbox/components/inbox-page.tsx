      ...
      <div className='flex min-w-0 flex-1 flex-col overflow-hidden'>
        <header className='shrink-0 border-b border-primary-soft bg-card px-4 py-3 md:px-6'>
          <div className='flex items-center gap-3'>
            ...
            <span
              className='hidden items-center gap-1.5 text-[11px] font-semibold text-emerald-700 sm:flex'
              role='status'
            >
        ...
        {inbox.error
          ? (
            <div
              className='mx-4 mt-3 flex shrink-0 items-center justify-between rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-semibold text-rose-800'
              role='alert'
            >
        ...
        {inbox.selectedConversation?.capabilities.permissionStatus === 'expired'
          ? (
            <div
              className='mx-4 mt-3 flex shrink-0 items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-900 md:mx-5'
              role='status'
            >
              <span>
                <strong>Quyền nhắn tin đã hết hạn.</strong> Bạn vẫn có thể xem lịch sử đã đồng bộ.
              </span>
        ...
        <div className='min-h-0 flex-1 overflow-hidden md:p-4'>
          <div className='grid h-full min-h-0 overflow-hidden bg-card md:grid-cols-[20rem_minmax(0,1fr)] md:rounded-xl md:border md:border-primary-soft md:shadow-sm 2xl:grid-cols-[20rem_minmax(0,1fr)_18rem]'>
            <div
              ...
              } min-h-0 border-r border-primary-soft md:block`}
            >
                  ...
                  <div className='flex h-full items-center justify-center bg-background-light p-8 text-center'>
                    <div>
                      ...
                      <p className='mt-1 text-xs text-foreground-muted'>
                        Nội dung và ngữ cảnh khách hàng sẽ xuất hiện tại đây.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <aside
              className='hidden min-h-0 border-l border-primary-soft 2xl:block'
              aria-label='Thông tin khách hàng và AI'
            >
              {inbox.selectedConversation
                ? (
      ...
      {isContextOpen && inbox.selectedConversation
        ? (
          <div
            ...
            role='presentation'
            onMouseDown={() => setIsContextOpen(false)}
          >
            <aside
              role='dialog'
              aria-modal='true'
            ...
            >
              <div className='flex h-14 items-center justify-between border-b border-primary-soft px-4'>
                <h2 id='inbox-context-title' className='text-sm font-extrabold text-foreground'>
                  Thông tin hội thoại
                </h2>
                <Button
                  autoFocus
