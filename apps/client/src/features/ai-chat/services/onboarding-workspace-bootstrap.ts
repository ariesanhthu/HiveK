...
import {
  isAgentWorkspaceData,
  saveAgentWorkspaceDemo,
} from "./workspace-demo-service";
...
function makeGenericChannel(
  ...
  const displayName =
    choice.accountLabel.trim() || `${businessName} — ${copy.label}`;
  ...
  return {
    ...
    role: copy.role,
    audience:
      audiences.length > 0 ? audiences : ["Khách hàng mục tiêu cần xác nhận"],
    contentPillars,
...
function applyRichDetails(
  ...
  replaceRichFact(facts, "fact-brand-name", "identity", "Tên thương hiệu", details.businessName, true, now, websiteUrl);
  replaceRichFact(facts, "fact-industry", "identity", "Lĩnh vực", details.industry, true, now, websiteUrl);
  replaceRichFact(facts, "fact-website", "contact", "Website chính thức", details.websiteUrl, false, now, websiteUrl);
  replaceRichFact(facts, "fact-mission", "positioning", "Mô tả và định hướng", details.summary, true, now, websiteUrl);
  replaceRichFact(facts, "fact-market", "positioning", "Thị trường chính", details.primaryMarket, true, now, websiteUrl);
  replaceRichFact(facts, "fact-address", "contact", "Địa chỉ", details.address, false, now, websiteUrl);
  replaceRichFact(facts, "fact-phone", "contact", "Hotline", details.phone, false, now, websiteUrl);
  replaceRichFact(facts, "fact-email", "contact", "Email liên hệ", details.email, false, now, websiteUrl);
  replaceRichFact(facts, "fact-voice", "voice", "Giọng thương hiệu", details.brandVoiceNotes, true, now, websiteUrl);
...
function readinessFor(
  ...
  const channelRatio =
    channels.length > 0 ? confirmedChannels / channels.length : 0;
  const unresolvedItems =
    facts.length - confirmedFacts + channels.length - confirmedChannels;
...
function genericStrategy(
  ...
  const audience =
    state.details.targetAudiences[0] || "nhóm khách hàng mục tiêu";
  const services =
    state.details.services.length > 0
  ...
  return {
    ...
    northStar: `Tạo nhu cầu phù hợp cho ${name} bằng nội dung hữu ích trong lĩnh vực ${industry}, đồng thời xây niềm tin với ${audience}.`,
    objectives: [
    ...
    priorities: [
      ...
      },
      {
        ...
        reason: "Chưa có connector analytics nên chưa thể dùng hiệu suất lịch sử để phân bổ nguồn lực.",
        owner: "Chủ workspace",
        dueDay: 14,
        status: "not_started",
      },
    ...
    contentPillars: [
      ...
      },
      {
        ...
        description: "Trình bày quy trình, đội ngũ và bằng chứng có nguồn; không tạo claim chưa kiểm chứng.",
        sharePercent: 35,
    ...
    provenance: [
      {
        ...
        url: null,
        observedAt: now,
        note: "Bản nháp dựa trên dữ liệu người dùng nhập và các nguồn đã chọn; cần duyệt trước khi vận hành.",
      },
    ],
  };
}
...
function genericSatellites(
  ...
  return [
    ...
    },
    {
      ...
      projectedImpact: "Tăng thời gian tiếp xúc với nội dung chuyên môn và lượng truy cập bền vững.",
      effort: "high",
      confidence: 0.58,
      dataQuality: "estimated",
      profile: {
    ...
    contentRules: [
      {
        ...
        instruction: "Mọi số liệu, kết quả hoặc tuyên bố so sánh phải có nguồn và thời điểm quan sát.",
        appliesTo: ["all"],
        level: "required",
        enabled: true,
      },
      {
        ...
        instruction: "Không dùng dữ liệu cá nhân hoặc tài sản riêng tư khi chưa có chấp thuận phù hợp.",
        appliesTo: ["all"],
        level: "required",
        enabled: true,
      },
      {
        ...
        instruction: "Claim về hiệu quả, giá, pháp lý hoặc sức khỏe phải được người có trách nhiệm duyệt.",
        appliesTo: ["all"],
        level: "required",
        enabled: true,
      },
    ...
    auditLog: [
      {
        ...
        objectLabel: name,
        detail: `Đã tạo hồ sơ từ ${resources.sources.length} nguồn được chọn và ${genericFacts(state, now).length} dữ kiện do người dùng cung cấp; không có hành động xuất bản.`,
      },
    ],
    tasks: [
      {
...
function richStudio(
  ...
  return {
    ...
    team: studio.team.map((member) => ({
      ...member,
      email:
        member.role === "owner" && details.email.trim()
          ? details.email.trim()
          : member.email,
    ...
    auditLog: [
      {
        ...
        detail: `Workspace chỉ sử dụng ${resources.sources.length} nguồn và ${resources.channels.length} hồ sơ kênh đã được người dùng chọn; không có hành động xuất bản.`,
      },
...
function buildGenericWorkspace(
  ...
  const workspace: AgentWorkspaceData = {
    ...
    brand: {
      ...
      primaryAudience:
        state.details.targetAudiences.length > 0
          ? [...state.details.targetAudiences]
