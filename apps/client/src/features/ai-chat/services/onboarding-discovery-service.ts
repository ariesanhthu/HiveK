  ...
  try {
    ...
    if (!query || !isTutorXQuery(query)) {
      ...
      return {
        matched: false,
        query,
        reason: 'Tôi chưa xác định được nguồn công khai đủ tin cậy từ thông tin này.',
      };
    }
    ...
    const sources: DiscoverySource[] = seed.sources.map((source, index) => ({
      ...
      statusMessage: 'Có thể đọc nội dung công khai; chưa có quyền xem insights hoặc xuất bản.',
      checkedAt,
    }));
