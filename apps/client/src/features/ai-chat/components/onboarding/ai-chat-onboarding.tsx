...
import { ChatWelcome, type ChatWelcomePrompt } from '@/features/ai-chat/components/chat-welcome';
...
const ONBOARDING_PROMPTS = [
  {
    id: 'quick-start',
    label: 'Bắt đầu nhanh',
    description: 'Dùng dữ liệu mẫu The TutorX để đi qua đầy đủ luồng connect và khởi tạo.',
    icon: 'link',
...
export function AiChatOnboarding({ flow, onReset }: AiChatOnboardingProps) {
  ...
  useEffect(() => {
    ...
    void import(
      ...
      .then(({ discoverOnboardingWorkspace }) => discoverOnboardingWorkspace(query))
      .then((result) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    if (activeStep) {
      const timer = window.setTimeout(() => {
        ...
        const noteByStep = {
          validate_sources: selectedSources > 0
          ...
          build_brand_profile:
            `Đã chuẩn bị ${completedDetailGroups} nhóm dữ kiện thương hiệu từ thông tin đã xác nhận.`,
          build_channel_profiles: selectedSources > 0
            ? `Đã chuẩn bị hồ sơ cho ${selectedSources} nguồn/kênh đã chọn.`
            : 'Chưa tạo hồ sơ kênh vì chưa có nguồn được kết nối.',
          suggest_satellites: 'Đã chuẩn bị các đề xuất kênh vệ tinh để người dùng xem xét.',
          draft_strategy: 'Đã soạn khung chiến lược ban đầu và giữ ở trạng thái chờ xác nhận.',
          prepare_analytics: 'Đã chuẩn bị cấu trúc dashboard và ghi rõ phạm vi dữ liệu hiện có.',
        } as const;
    ...
    if (allComplete) {
      ...
      const timer = window.setTimeout(() => {
        ...
        void import(
          ...
          .then(({ materializeOnboardingWorkspace }) => materializeOnboardingWorkspace(state))
          .then((saved) => {
            if (cancelled) return;
            if (!saved) {
              throw new Error(
  ...
  const completedSetupSteps = state.stage === 'welcome' || state.stage === 'mode'
    ? 0
    ...
    ? 4
    : 3;
  const headerStatus = state.stage === 'welcome'
  ...
  const composerPlaceholder = state.stage === 'welcome'
          ...
          {state.stage === 'welcome'
            ...
            : (
              <div className='mx-auto w-full max-w-3xl space-y-4 px-3 py-5 sm:px-5 sm:py-7'>
                {state.discovery.query ? <UserMessage>{state.discovery.query}</UserMessage> : null}

                {state.stage === 'mode'
                  ? (
                    <>
                      <AgentMessage eyebrow='Cách Agent làm việc'>
                        Tôi đã ghi nhận nguồn bạn muốn kết nối. Trước khi quét dữ liệu, hãy chọn để
                        tôi tự động hoàn tất toàn bộ hoặc dừng lại xin xác nhận sau từng đầu việc.
                      </AgentMessage>
                  ...
                  ? (
                    <>
                      <AgentMessage eyebrow='Đang thực hiện'>
                        Tôi sẽ tìm nguồn công khai trước. Nếu không nhận diện được hoặc không thể
                        thêm nguồn, tôi sẽ hỏi bạn đúng những trường còn thiếu.
                      </AgentMessage>
                  ...
                  ? (
                    <>
                      <AgentMessage eyebrow='Đã có kết quả'>
                        Tôi đã tìm thấy các trang có cùng nhận diện. Hãy chọn nguồn bạn muốn dùng;
                        hiện tại tôi chỉ đọc phần công khai và không có quyền đăng bài.
                      </AgentMessage>
                ...
                {state.stage === 'details'
                  ? (
                    <>
                      <AgentMessage
                        eyebrow={state.discovery.status === 'failed'
                          ? 'Tôi cần bạn hỗ trợ'
                          : 'Bước xác nhận'}
                      >
                        {state.discovery.status === 'failed'
                          ? (
                            <>
                              Tôi chưa tự nối được thông tin này với một nguồn đủ tin cậy. Hãy nhập
                              một URL website/fanpage cùng tên thương hiệu, lĩnh vực và thị trường;
                              các trường khác có thể bổ sung sau.
                            </>
                          )
                          : (
                            <>
                              Tôi đã điền các trường đọc được từ nguồn. Bạn chỉ cần sửa chỗ chưa
                              đúng rồi xác nhận để tôi tạo toàn bộ hồ sơ liên quan.
                            </>
                          )}
                      </AgentMessage>
