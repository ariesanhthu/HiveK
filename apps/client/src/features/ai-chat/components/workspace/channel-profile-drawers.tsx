      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <Field id="channel-pillars" label="Trụ cột nội dung" hint="Mỗi dòng là một trụ cột.">
          <textarea
            ...
            onChange={(event) => setDraft({ ...draft, contentPillars: splitList(event.target.value) })}
            className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20"
          />
        </Field>
  ...
  return (
    ...
    >
      <div className="flex flex-wrap gap-2">
        <Badge variant={satellite.status === "accepted" || satellite.status === "active" ? "success" : "warning"}>
          {satellite.status === "accepted" ? "Đã chấp nhận" : satellite.status === "active" ? "Đang vận hành" : "Đang đề xuất"}
        </Badge>
        <Badge variant="secondary">Tin cậy {Math.round(satellite.confidence * 100)}%</Badge>
        <Badge variant="outline">Nỗ lực {satellite.effort === "low" ? "thấp" : satellite.effort === "medium" ? "vừa" : "cao"}</Badge>
      </div>

      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <Field id="satellite-name" label="Tên trang/kênh">
          <Input id="satellite-name" data-drawer-initial-focus value={draft.profile.displayName} onChange={(event) => updateProfile({ displayName: event.target.value })} />
        </Field>
        <Field id="satellite-handle" label="Handle đề xuất">
          <Input id="satellite-handle" value={draft.profile.proposedHandle} onChange={(event) => updateProfile({ proposedHandle: event.target.value })} />
        </Field>
        <div className="sm:col-span-2">
          <Field id="satellite-bio" label="Bio đã điền">
            <textarea id="satellite-bio" rows={4} value={draft.profile.bio} onChange={(event) => updateProfile({ bio: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20" />
          </Field>
        </div>
        <Field id="satellite-role" label="Vai trò kênh">
          <Input id="satellite-role" value={draft.profile.role} onChange={(event) => updateProfile({ role: event.target.value })} />
        </Field>
        <Field id="satellite-cadence" label="Nhịp đăng">
          <Input id="satellite-cadence" value={draft.profile.cadence} onChange={(event) => updateProfile({ cadence: event.target.value })} />
        </Field>
        <div className="sm:col-span-2">
          <Field id="satellite-tone" label="Giọng điệu">
            <Input id="satellite-tone" value={draft.profile.tone} onChange={(event) => updateProfile({ tone: event.target.value })} />
          </Field>
        </div>
        <Field id="satellite-audience" label="Đối tượng" hint="Mỗi dòng là một nhóm.">
          <textarea id="satellite-audience" rows={5} value={joinList(draft.profile.audience)} onChange={(event) => updateProfile({ audience: splitList(event.target.value) })} className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20" />
        </Field>
        <Field id="satellite-pillars" label="Trụ cột nội dung" hint="Mỗi dòng là một trụ cột.">
          <textarea id="satellite-pillars" rows={5} value={joinList(draft.profile.contentPillars)} onChange={(event) => updateProfile({ contentPillars: splitList(event.target.value) })} className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20" />
        </Field>
        <Field id="satellite-formats" label="Định dạng" hint="Mỗi dòng là một định dạng.">
          <textarea id="satellite-formats" rows={4} value={joinList(draft.profile.formats)} onChange={(event) => updateProfile({ formats: splitList(event.target.value) })} className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20" />
        </Field>
        <Field id="satellite-guardrails" label="Điều không được làm" hint="Mỗi dòng là một quy tắc.">
          <textarea id="satellite-guardrails" rows={4} value={joinList(draft.profile.guardrails)} onChange={(event) => updateProfile({ guardrails: splitList(event.target.value) })} className="w-full rounded-xl border border-slate-200 bg-card px-3 py-2 text-sm leading-6 text-foreground outline-none focus:border-amber-300 focus:ring-2 focus:ring-primary/20" />
        </Field>
        <div className="sm:col-span-2">
          <Field id="satellite-cta" label="CTA mặc định">
            <Input id="satellite-cta" value={draft.profile.cta} onChange={(event) => updateProfile({ cta: event.target.value })} />
          </Field>
        </div>
      </div>
