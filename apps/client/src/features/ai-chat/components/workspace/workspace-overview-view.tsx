...
export function WorkspaceOverviewView({
  ...
  const activeTasks = [...data.studio.tasks]
    ...
    .slice(0, 4);
  const currentPhase = data.strategy.phases.find((phase) =>
        ...
        <div className='grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-center'>
          <div>
            ...
            <p className='mt-3 max-w-2xl text-sm leading-6 text-foreground-muted'>
              {data.brand.readiness.summary}{' '}
              Mọi dữ kiện quan trọng đều có nguồn hoặc nhãn chất lượng dữ liệu để bạn đối chiếu
              trước khi xác nhận.
            </p>
            <div className='mt-5 flex flex-wrap gap-2'>
              <Badge variant='success'>
                <CheckCircle2 className='mr-1 size-3.5' aria-hidden />
                {data.brand.readiness.allFacts.confirmed}/{data.brand.readiness.allFacts.total}{' '}
                dữ kiện đã duyệt
              </Badge>
            ...
            <dl className='mt-4 grid grid-cols-2 gap-3 text-xs'>
              <div className='rounded-xl bg-slate-50 p-3'>
                ...
                <dd className='mt-1 font-extrabold text-foreground'>
                  {data.brand.readiness.requiredFacts.confirmed}/{data.brand.readiness.requiredFacts
                    .total}
                </dd>
              </div>
              <div className='rounded-xl bg-slate-50 p-3'>
                ...
                <div key={source.id} className='flex items-center gap-3 py-3 first:pt-0 last:pb-0'>
                  ...
                  <div className='min-w-0 flex-1'>
                    ...
                    <p className='mt-0.5 text-[11px] text-foreground-muted'>
                      {source.permission === 'read_publish' ? 'Đọc và xuất bản' : 'Chỉ đọc'} ·{' '}
                      {source.status === 'connected'
                        ? 'Đã kết nối'
                        : source.status === 'public_only'
                        ? 'Nguồn công khai'
                        : 'Cần chú ý'}
                    </p>
                  </div>
      ...
      <div className='grid gap-3 sm:grid-cols-3'>
        ...
        >
          ...
          <p className='mt-3 text-sm font-extrabold text-foreground'>
            {data.channels.length} kênh hiện có
          </p>
          <p className='mt-1 text-xs text-foreground-muted'>
            Xem quyền, vai trò và hồ sơ từng kênh
          </p>
        </button>
        <button
          type='button'
        ...
        >
          ...
          <p className='mt-3 text-sm font-extrabold text-foreground'>
            {data.satellites.length} hồ sơ vệ tinh
          </p>
          <p className='mt-1 text-xs text-foreground-muted'>
            Đã điền bio, vai trò, nội dung và CTA
          </p>
        </button>
        <button
          type='button'
        ...
        >
          ...
          <p className='mt-3 text-sm font-extrabold text-foreground'>
            {data.analytics.dataCompletenessPercent}% dữ liệu phân tích
          </p>
          <p className='mt-1 text-xs text-foreground-muted'>
            Insight luôn kèm mức tin cậy và số mẫu
          </p>
        </button>
      </div>
    </div>
