...
export function StrategyEditorDrawer({
  ...
  const { dialogRef, requestClose } = useDrawerDialog({
    ...
    onClose,
    discardMessage:
      "Bạn có thay đổi chiến lược chưa lưu. Đóng và bỏ các thay đổi này?",
  });
      ...
      >
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-4 py-4 sm:px-6">
          <div className="min-w-0">
            ...
            >
              Chỉnh các quyết định cốt lõi và tiến độ ưu tiên. Lưu bản nháp
              không đồng nghĩa với xác nhận hay xuất bản.
            </p>
          </div>
          <Button
            variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-6">
            ...
            <fieldset className="mt-6 rounded-2xl border border-slate-200 p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-foreground-muted">
                Cập nhật tiến độ từng đầu việc; thay đổi sẽ được lưu cùng phiên bản
                chiến lược.
              </p>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <select
                          id={selectId}
                          value={
                            draft.priorityStatuses[priority.id] ?? priority.status
                          }
                          onChange={(event) =>
                            setDraft((current) => ({
                              ...current,
