...
function getTrendSummary(points: AnalyticsTrendPoint[]) {
  ...
  return `Trong ${points.length} kỳ dữ liệu, lượt tiếp cận ${direction}. Tương tác cao nhất ở ${peak.label}, đạt ${COMPACT_NUMBER.format(peak.engagements)}.`;
}
...
function getPillarSummary(pillars: ContentPillarPerformance[]) {
  ...
  return `${strongest.label} có tỷ lệ tương tác cao nhất: ${strongest.engagementRate.toLocaleString("vi-VN")}% trên ${strongest.posts} bài. Dùng kết quả như tín hiệu tham khảo, không tự động thay đổi kế hoạch.`;
}
              ...
              >
                ...
                >
                  ...
                  <ChartTooltip
                    cursor={false}
                    content={
                      <ChartTooltipContent valueFormatter={formatCompactNumber} />
                    }
                  />
                  <Area
                    yAxisId="volume"
