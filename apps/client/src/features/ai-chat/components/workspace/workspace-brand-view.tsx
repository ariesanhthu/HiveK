...
export function WorkspaceBrandView({
  ...
  const status: StatusFilter =
    statusFromUrl === "confirmed" || statusFromUrl === "needs_review"
      ? statusFromUrl
      : "all";
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
      ...
      <section className="sticky top-0 z-10 rounded-2xl border border-slate-200 bg-card/95 p-3 shadow-sm backdrop-blur sm:p-4" aria-label="Bộ lọc dữ kiện">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          <div className="flex items-center gap-2 overflow-x-auto overscroll-x-contain pb-1 lg:pb-0">
            ...
            {(["all", "needs_review", "confirmed"] as const).map((filter) => (
              ...
              >
                {filter === "all" ? "Tất cả" : filter === "needs_review" ? "Cần xác nhận" : "Đã xác nhận"}
              </button>
            ))}
          </div>
        </div>
        <div className="mt-3 flex gap-1.5 overflow-x-auto overscroll-x-contain pb-1">
          <button
            ...
            className={cn(
              ...
              category === "all" ? "bg-slate-900 text-white" : "bg-slate-100 text-foreground-muted hover:text-foreground"
            )}
          >
            Mọi nhóm
          </button>
          {CATEGORY_ORDER.map((categoryId) => (
            <button
              ...
              className={cn(
                "h-8 shrink-0 rounded-lg px-2.5 text-[11px] font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40",
                category === categoryId ? "bg-slate-900 text-white" : "bg-slate-100 text-foreground-muted hover:text-foreground"
              )}
            >
      ...
      {groupedFacts.length > 0 ? (
          ...
          {groupedFacts.map((group) => (
            <section key={group.id} aria-labelledby={`fact-group-${group.id}`}>
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 id={`fact-group-${group.id}`} className="text-base font-extrabold text-foreground">
                  {CATEGORY_LABELS[group.id]}
                </h2>
                <span className="text-xs font-semibold text-foreground-muted">{group.facts.length} trường</span>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {fact.isRequired ? <span className="text-[10px] font-bold text-red-600">Bắt buộc</span> : null}
                          </div>
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-100 pt-3 text-[11px] text-foreground-muted">
                        ...
                        {primarySource ? (
                          <>
                            <span aria-hidden>·</span>
                            <span className="max-w-48 truncate">Nguồn: {primarySource.label}</span>
                          </>
                        ) : null}
                      </div>

                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button variant="outline" size="sm" onClick={() => onEditFact(fact.id)}>
                          Sửa hoặc xem nguồn
                        </Button>
                        {fact.status === "needs_review" ? (
                          <Button size="sm" onClick={() => onConfirmFact(fact.id)}>
        ...
        <section className="rounded-2xl border border-slate-200 bg-card px-5 py-10 text-center">
          ...
          <h2 className="mt-3 text-base font-extrabold text-foreground">Không có dữ kiện phù hợp</h2>
          <p className="mt-1 text-sm text-foreground-muted">Hãy xóa từ khóa hoặc thay đổi bộ lọc.</p>
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => {
