      ...
      >
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <StudioToggle
            ...
            onChange={(checked) =>
              updateGovernance({ requireFactSources: checked })
            }
          />
          <StudioToggle
            ...
            onChange={(checked) =>
              updateGovernance({ requireHumanReviewForClaims: checked })
            }
          />
        </div>
              ...
              >
                <div className="grid gap-4 lg:grid-cols-2">
                  <StudioToggle
                    ...
                    onChange={(checked) =>
                      updateContentRule(rule.id, { enabled: checked })
                    }
                  />
                  <StudioSelectField
                  ...
                  <StudioTextField
                    ...
                    value={rule.name}
                    onChange={(value) =>
                      updateContentRule(rule.id, { name: value })
                    }
                    required
                  />
                  <StudioListField
                    ...
                    onChange={(values) =>
                      updateContentRule(rule.id, { appliesTo: values })
                    }
                    rows={3}
                  />
                  <div className="lg:col-span-2">
                    <StudioTextAreaField
                      ...
                      onChange={(value) =>
                        updateContentRule(rule.id, { instruction: value })
                      }
                      rows={4}
                      required
                    />
              ...
              >
                <div className="grid gap-4 lg:grid-cols-2">
                  <StudioToggle
                    ...
                    checked={rule.enabled}
                    onChange={(checked) =>
                      updateChannelRule(rule.id, { enabled: checked })
                    }
                  />
                  <StudioTextField
                    ...
                    value={rule.role}
                    onChange={(value) =>
                      updateChannelRule(rule.id, { role: value })
                    }
                  />
                  <StudioTextField
                    ...
                    value={rule.tone}
                    onChange={(value) =>
                      updateChannelRule(rule.id, { tone: value })
                    }
                  />
                  <StudioTextField
                    ...
                    value={rule.cadence}
                    onChange={(value) =>
                      updateChannelRule(rule.id, { cadence: value })
                    }
                  />
                  <StudioListField
                    ...
                    onChange={(values) =>
                      updateChannelRule(rule.id, { contentPillars: values })
                    }
                  />
                  <StudioTextAreaField
                    ...
                    value={rule.cta}
                    onChange={(value) =>
                      updateChannelRule(rule.id, { cta: value })
                    }
                    rows={4}
                  />
                </div>
          ...
          <div className="space-y-2">
            {config.sources.map((source, index) => (
              <StudioDisclosure
                key={source.id}
                title={source.name}
                description={`${SOURCE_TYPE_LABELS[source.type]} · Đồng bộ ${formatSyncTime(source.lastSyncedAt)}`}
                defaultOpen={index === 0}
                meta={
              ...
              >
                {source.status === "needs_reconnect" ? (
                  <div className="mb-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs leading-5 text-amber-900">
                    ...
                    <p>
                      Nguồn này cần được kết nối lại. Dữ liệu đã lưu vẫn còn nhưng thay đổi mới chưa thể đồng bộ.
                    </p>
                  </div>
                ) : null}
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <div className="lg:col-span-2">
                    <StudioToggle
                      ...
                      checked={
                        source.includeInAgentKnowledge ?? source.canRead
                      }
                      onChange={(checked) =>
                        updateSource(source.id, {
                          includeInAgentKnowledge: checked,
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-foreground-muted">
              Chỉ cấp quyền ghi hoặc đăng cho nguồn thực sự cần xuất bản. Quyền đọc vẫn đủ để phân tích và gợi ý nội dung.
            </p>
          </div>
        </div>
      </div>
