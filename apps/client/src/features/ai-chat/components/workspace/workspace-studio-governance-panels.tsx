    ...
    <div className='space-y-4'>
            ...
            <div className='flex items-start gap-3'>
              ...
              <div>
                ...
                <p className='mt-1 text-xs leading-5 text-foreground-muted'>
                  Thay đổi giá, ưu đãi, cam kết, lịch hoặc tài khoản đích sau khi duyệt sẽ đưa nội
                  dung trở lại hàng chờ kiểm tra.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        {config.approvalWorkflows.length
                ...
                >
                  ...
                  <div className='grid gap-4 lg:grid-cols-2'>
                    <StudioToggle
                      ...
                      onChange={(checked) => updateWorkflow(workflow.id, { enabled: checked })}
                    />
                    <StudioTextField
                      id={`${idPrefix}-workflow-name-${workflow.id}`}
                      label='Tên workflow'
                      value={workflow.name}
                      onChange={(value) => updateWorkflow(workflow.id, { name: value })}
                      required
                    />
                    <StudioListField
                      ...
                      onChange={(values) => updateWorkflow(workflow.id, { contentTypes: values })}
                      rows={3}
                    />
                    <StudioListField
                      ...
                      onChange={(values) => updateWorkflow(workflow.id, { channelIds: values })}
                      rows={3}
                    />
                    <StudioNumberField
                      ...
                      onChange={(value) => updateWorkflow(workflow.id, { slaHours: value })}
                      min={1}
                      max={720}
                    />
                    <StudioSelectField
                        ...
                        >
                          ...
                          >
                            <Button
                              ...
                              size='icon'
                              className='size-9'
                              onClick={() => moveWorkflowStep(workflow, stepIndex, -1)}
                            ...
                            <Button
                              ...
                              onClick={() => removeWorkflowStep(workflow, stepIndex)}
                              disabled={workflow.steps.length <= 1}
                              aria-label={`Xóa bước duyệt ${step.order}`}
                              title={workflow.steps.length <= 1
          ...
          : (
            <StudioEmptyState>
              Chưa có workflow. Nội dung chỉ có thể được chuẩn bị và đăng thủ công cho đến khi quy
...
export function AgentLearningPanel({
  ...
  function updateLearningRule(
    ...
    onChange(
      'learningRules',
      config.learningRules.map((rule) => rule.id === ruleId ? { ...rule, ...patch } : rule),
    );
  }
      ...
      >
        <div className='grid gap-4 lg:grid-cols-2'>
          <StudioSelectField
            ...
            onChange={(value) => onChange('agentMode', value as WorkspaceStudioConfig['agentMode'])}
            options={[
              {
                value: 'guided',
          ...
          <div className='rounded-xl border border-primary-soft bg-muted/45 px-4 py-3'>
            <div className='flex items-start gap-3'>
              ...
              <div>
                ...
                <p className='mt-1 text-xs leading-5 text-foreground-muted'>
                  Agent có thể chuẩn bị và đề xuất thao tác; duyệt, thay đổi quyền và xuất bản vẫn
                  cần người có quyền xác nhận.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <div className='grid gap-3 lg:grid-cols-2'>
          ...
          <StudioToggle
            ...
            onChange={(checked) => updateLearning({ learnFromApprovedEdits: checked })}
          ...
          <StudioToggle
            ...
            onChange={(checked) => updateLearning({ learnFromPerformance: checked })}
            disabled={!config.learningEnabled}
          />
          <StudioToggle
            ...
            onChange={(checked) => updateLearning({ requireApprovalForStableRules: checked })}
            disabled={!config.learningEnabled}
          />
          <StudioToggle
            ...
            onChange={(checked) => updateLearning({ preserveRejectedRules: checked })}
            disabled={!config.learningEnabled}
          />
          <StudioNumberField
                ...
                >
                  <div className='grid gap-4 lg:grid-cols-2'>
                    <StudioToggle
                      ...
                      onChange={(checked) => updateLearningRule(rule.id, { enabled: checked })}
                    ...
                    <StudioTextField
                      ...
                      label='Nội dung rule'
                      value={rule.label}
                      onChange={(value) => updateLearningRule(rule.id, { label: value })}
                    />
                    <StudioTextAreaField
                      id={`${idPrefix}-learning-rule-evidence-${rule.id}`}
                      label='Tóm tắt bằng chứng'
                      value={rule.evidence}
                      onChange={(value) => updateLearningRule(rule.id, { evidence: value })}
                      rows={4}
                    />
                  </div>
                </StudioDisclosure>
...
export function TeamNotificationsAuditPanel({
  ...
  function updateMember(memberId: string, patch: Partial<TeamMember>): void {
    onChange(
      'team',
      config.team.map((member) => member.id === memberId ? { ...member, ...patch } : member),
    );
  }
  ...
  function removeMember(member: TeamMember): void {
    ...
    onChange(
      ...
      config.approvalWorkflows.map((workflow) => ({
        ...workflow,
        fallbackMemberId: workflow.fallbackMemberId === member.id
          ? null
  ...
  function updateNotification(
    ...
    onChange(
      'notifications',
      config.notifications.map((rule) => rule.id === ruleId ? { ...rule, ...patch } : rule),
    );
  }
  ...
  function updateTask(taskId: string, patch: Partial<WorkspaceTask>): void {
    onChange(
      'tasks',
      config.tasks.map((task) => task.id === taskId ? { ...task, ...patch } : task),
    );
  }
            ...
            <div className='space-y-2'>
              {config.team.map((member, index) => (
                <StudioDisclosure
                  key={member.id}
                  title={member.name}
                  description={`${member.email} · ${ROLE_LABELS[member.role]} · ${
                ...
                >
                  ...
                  <div className='grid gap-4 lg:grid-cols-2'>
                    ...
                    <StudioTextField
                      ...
                      type='email'
                      value={member.email}
                      onChange={(value) => updateMember(member.id, { email: value })}
                      required
                      autoComplete='email'
                    />
                    <StudioSelectField
                    ...
                    <div className='lg:col-span-2'>
                      <StudioListField
                        ...
                        onChange={(values) => updateMember(member.id, { channelIds: values })}
                ...
                >
                  ...
                  <div className='grid gap-4 lg:grid-cols-2'>
                    <StudioToggle
                      ...
                      checked={rule.enabled}
                      onChange={(checked) => updateNotification(rule.id, { enabled: checked })}
                    />
                    <StudioSelectField
                    ...
                    <StudioTextField
                      ...
                      label='Sự kiện'
                      value={rule.event}
                      onChange={(value) => updateNotification(rule.id, { event: value })}
                    />
                ...
                >
                  <div className='grid gap-4 sm:grid-cols-2'>
                    ...
                    <StudioSelectField
                      ...
                      onChange={(value) => updateTask(task.id, { assigneeId: value || null })}
                      options={[
                        { value: '', label: 'Chưa chỉ định' },
                        ...config.team.map((member) => ({
                          value: member.id,
