          ...
          <ChartContainer config={nicheConfig} className="h-72">
            <PieChart>
              ...
              <Pie
                ...
                paddingAngle={3}
              >
                {nicheEngagement.map((entry) => (
                  <Cell key={entry.niche} fill={entry.fill} />
                ))}
              </Pie>
            </PieChart>
          </ChartContainer>
          ...
          <ChartContainer config={scatterConfig} className="h-72">
            <ScatterChart>
              ...
              <XAxis
                ...
                type="number"
              />
              <YAxis dataKey="kolScore" name="Score" tickLine={false} axisLine={false} type="number" />
        ...
        <CardContent>
          <ChartContainer config={audienceConfig} className="h-72">
            <BarChart data={audienceTree} layout="vertical" margin={{ left: 16 }}>
              ...
              <XAxis type="number" tickFormatter={formatCompact} tickLine={false} axisLine={false} />
              <YAxis dataKey="label" type="category" tickLine={false} axisLine={false} width={76} />
              <ChartTooltip content={<ChartTooltipContent valueFormatter={(value) => formatCompact(Number(value))} />} />
