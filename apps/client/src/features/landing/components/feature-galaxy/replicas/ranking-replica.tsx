...
const ROWS = [
  { rank: 1, delta: "same", name: "Khoai Lang Thang", handle: "@Khoailangthang", niche: "Du lịch", followers: "2.7M", score: 98.4, badge: "Top 1" },
  { rank: 2, delta: "up", name: "MixiGaming", handle: "@mixigaming3con", niche: "Gaming", followers: "8.2M", score: 96.1, badge: "Ưu tú" },
  { rank: 3, delta: "up", name: "Sun HT", handle: "@SunHT", niche: "Fitness", followers: "1.2M", score: 94.7, badge: "Top 10" },
  { rank: 4, delta: "down", name: "Giang Ơi", handle: "@GiangOi", niche: "Đời sống", followers: "2.2M", score: 91.3, badge: null },
];
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          {["Tất cả ngách", "Tất cả nền tảng"].map((f) => (
            <span key={f} className="rounded-full border border-primary-soft bg-muted px-2.5 py-1 text-[10px] font-semibold text-foreground-muted">
              {f}
            </span>
          ))}
        </div>
                ...
                <tr key={r.handle}>
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`material-symbols-outlined text-[13px] ${d.className}`} aria-hidden>
                        {d.icon}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="hidden px-3 py-2 text-foreground-muted sm:table-cell">{r.niche}</td>
                  <td className="hidden px-3 py-2 text-foreground-muted sm:table-cell">{r.followers}</td>
                  <td className="px-3 py-2 text-right font-black text-primary">{r.score}</td>
                  <td className="px-3 py-2 text-right">
                    {r.badge && (
                      <span className={`rounded-full border px-2 py-0.5 text-[9px] font-bold ${BADGE_CLASS[r.badge]}`}>
                        {r.badge}
                      </span>
                    )}
                  </td>
