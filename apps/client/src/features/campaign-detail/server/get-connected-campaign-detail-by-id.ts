...
function mapBackendCampaignToDetail(campaign: BackendCampaign): CampaignDetail {
  ...
  const scheduledPosts =
    campaign.schedule?.timeline?.reduce(
  ...
  return {
    ...
    creators: participants.slice(0, 6).map((participant, index) => ({
      ...
      name: participant.kolProfileId
        ...
        : `Creator ${index + 1}`,
      avatarUrl: `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(participant.id)}`,
    ...
    recentContent:
      campaign.rawContents?.slice(0, 3).map((content, index) => ({
        id: content.fileId ?? `content-${index}`,
        thumbnailUrl:
          ACTIVE_CAMPAIGNS[index % ACTIVE_CAMPAIGNS.length]?.image ??
