...
function buildFromSlug(slug: string): KolVerificationCertificate {
  ...
  const name = known?.displayName ?? pick(NAME_POOL, h);
  const role = known?.roleLabel ?? pick(ROLE_POOL, h >> 3);
  const verificationId = known?.verificationId
  ...
  const competencies = known?.competencies ?? [
  ...
  const partnerFeedback = known?.partnerFeedback ?? [
    {
      id: 'fb-a',
      quote: q0[0],
      partnerName: q0[1],
