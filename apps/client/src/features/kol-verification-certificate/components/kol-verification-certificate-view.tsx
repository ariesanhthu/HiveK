...
import { BadgeCheck, CheckCircle2, History, Quote, Shield, Sparkles, Star } from 'lucide-react';
  ...
  return (
    <div className='flex gap-0.5' aria-label={`Rating ${rating} of 5`}>
      {Array.from({ length: 5 }, (_, i) => {
        ...
        const isPartial = !isFull && i === Math.floor(rating) && rating % 1 >= 0.25;
        return (
          <Star
            key={i}
            className={cn(
        ...
        <div className='space-y-10 p-6 md:p-10'>
          <header className='space-y-4 text-center'>
            ...
            <p className='mx-auto max-w-2xl text-sm text-foreground-muted md:text-base'>
              This document validates authenticity, performance metrics, and verified track record
              for the creator profile below.
            </p>
          </header>
