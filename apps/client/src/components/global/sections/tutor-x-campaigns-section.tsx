...
import { TUTOR_X_CAMPAIGNS } from "@/data/mock-data";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * Tutor X version of the "Active Campaigns" section.
 * Structurally identical to ActiveCampaignsSection — only the data source,
 * heading, and description have changed to reflect the current ICP
 * (gia sư, homestay, cộng tác viên).
 */
export const TutorXCampaignsSection: React.FC = () => {
    ...
    >
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-12 flex flex-col items-center gap-6 text-center sm:flex-row sm:items-end sm:justify-between sm:text-left">
          <div className="max-w-xl">
            <h2 className="text-3xl font-black text-foreground">
              Cơ hội hợp tác nổi bật
            </h2>
            <p className="mt-4 text-muted">
              Các vị trí gia sư và homestay đang mở tuyển cộng tác viên
            </p>
          </div>
          <Link
            href="/campaigns"
        ...
        <div className="group relative">
          ...
          >
            {TUTOR_X_CAMPAIGNS.map((campaign) => (
              <div 
                key={campaign.id} 
