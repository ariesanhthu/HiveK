...
function scoreCandidate(candidate: KolCandidate): KolKocRecommendation {
  ...
  const overallMatchScore =
    ...
    riskScore * 0.2;
  const suggestedCampaignRole: FunnelStage | "review" =
    candidate.type === "KOC" ? "review" : "awareness";
  ...
  return {
    ...
    reason: `${candidate.name} has a strong fit score and efficient expected CPA for this mock campaign.`,
    evidence: [
    ...
    concerns:
      candidate.estimatedCostPerPostUsd > 1000
...
export async function matchKolKocWorkflow(
  ...
  const cacheKey = `agentic:kol-match:${request.campaignId}:${request.limit}:${request.filters.platforms.join("-")}:${request.filters.minFollowers ?? 0}:${request.filters.maxEstimatedCost ?? 0}`;
  ...
  const recommendations = candidates
    .filter((candidate) => {
      const meetsFollowers =
        request.filters.minFollowers === undefined ||
        candidate.followers >= request.filters.minFollowers;
      const meetsCost =
        request.filters.maxEstimatedCost === undefined ||
